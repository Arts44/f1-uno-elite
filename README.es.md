...
Cada demo la genera `capture_demos.py` con el mismo seed determinista que las capturas fijas, **idénticas byte a byte entre ejecuciones**: cada animación se congela en una fase elegida antes de cada disparo. Los GIF van a 33,3 fps; la versión a 60 fps está al lado: [añadido rápido](screenshots/demo-quick-add.mp4) · [navegación](screenshots/demo-nav.mp4) · [insignias](screenshots/demo-badges.mp4) · [temporadas](screenshots/demo-seasons.mp4).
...
- **La nota de Codacy es A — y uno de sus cuatro objetivos de calidad está en rojo.** Medido en el commit `e19d1fa`, 121 archivos, 9 741 líneas. En verde: la duplicación al 6 % (objetivo 10 %) y la cobertura al 69 % — la medición de Codacy y la local (`npm run test:cov`, 69,10 %) coinciden esta vez, frente a un objetivo del 60 %: nueve puntos de margen en lugar de 2,7. En rojo: la complejidad, con **el 44 % de los archivos analizados por encima del umbral** frente a un objetivo del 10 % — el ratio *subió* tras dividir `cloud.js` en cuatro, lo que dice mucho de la métrica: cuenta *archivos*, así que penaliza una base hecha de pocos módulos grandes. Es un hecho sobre la medición, no una excusa; `badges.js` hace realmente demasiado. Y una cuarta cifra que el distintivo oculta: **11 issues abiertas**, de las cuales **8 están en los scripts de captura añadidos para este README** (`subprocess` y `exec` en `capture_demos.py` — herramientas de desarrollo, no código publicado), 2 variables sin usar en `account.js` y 1 falso positivo en `cloud-auth.js`.
