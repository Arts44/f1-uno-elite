...
- **La página de inicio lleva un contador de audiencia sin cookies; la aplicación no carga nada.** La portada en la raíz del dominio incluye Cloudflare Web Analytics — sin cookies, sin rastreo entre sitios, solo recuentos de visitas. La aplicación (`/app/`) sigue cargando **cero recursos externos**, y una batería de tests rompe la build si aparece uno solo. La única excepción permitida está nombrada, archivo por archivo, en el mismo test.
- **La página de inicio tiene un presupuesto de peso medido — 120 KB, garantizado por un test.** El propio contador de Cloudflare reveló su lentitud: sus capturas eran el elemento LCP a 5 091 ms en el P90 de tráfico real, el 89 % del peso de la página. Corregido con variantes WebP dimensionadas para su visualización real, `fetchpriority` en el LCP y carga diferida en la captura oculta bajo 720 px — medido, no deducido: móvil de 277 KB a **73 KB**, escritorio a **99 KB**. Las variantes salen del mismo script determinista que las del README. La ironía merece anotarse: el único script externo de la página es precisamente lo que permitió descubrir que era lenta.
