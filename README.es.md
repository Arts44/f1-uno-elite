...
| Tests | **Runner de tests integrado en Node** (`node --test`) — 944 tests, sin framework de test |
...
**Solución:** la lógica se factorizó para ser independiente del navegador y está cubierta por **944 tests en el runner integrado de Node** — sin dependencias de test, sin red real. La CI también reconstruye el bundle y falla si el artefacto commiteado está obsoleto.

---

### Qué cubren las pruebas — y qué no

944 pruebas sobre el ejecutor integrado de Node, sin framework. Ser preciso sobre el límite importa más que el número:
...
npm test        # 944 tests, node --test, sin framework
```
