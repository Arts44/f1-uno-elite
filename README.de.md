...
## ▶️ **[Live ausprobieren → arts44.dev](https://arts44.dev/)**
...
| Tests | **Nodes eingebauter Test-Runner** (`node --test`) — 955 Tests, kein Test-Framework |
...
**Lösung:** Die Logik wurde browserfrei faktorisiert und wird von **955 Tests auf Nodes eingebautem Runner** abgedeckt — keine Testabhängigkeiten, kein echtes Netzwerk. Die CI baut zudem das Bundle neu und schlägt fehl, wenn das committete Artefakt veraltet ist.

---

### Was die Tests abdecken — und was nicht

955 Tests auf Nodes eingebautem Runner, ohne Framework. Genauigkeit über die Grenze zählt mehr als die Zahl:
...
npm test        # 955 Tests, node --test, ohne Framework
```
...
- **Die Startseite trägt einen cookielosen Besucherzähler; die App lädt nichts.** Die Schaufensterseite an der Domain-Wurzel enthält Cloudflare Web Analytics — keine Cookies, kein seitenübergreifendes Tracking, nur Besuchszahlen. Die Anwendung (`/app/`) lädt weiterhin **null externe Ressourcen**, und eine Testsuite bricht den Build, sobald eine einzige auftaucht. Die einzige erlaubte Ausnahme ist im selben Test Datei für Datei benannt.
- **Die Startseite hat ein gemessenes Gewichtsbudget — 120 KB, von einem Test gehalten.** Cloudflares eigener Besucherzähler deckte die Langsamkeit auf: Ihre Screenshots waren das LCP-Element mit 5 091 ms im P90 echten Traffics, 89 % des Seitengewichts. Behoben durch WebP-Varianten in ihrer tatsächlichen Anzeigegröße, `fetchpriority` auf dem LCP und Lazy Loading für das unter 720 px verborgene Bild — gemessen, nicht hergeleitet: mobil von 277 KB auf **73 KB**, Desktop auf **99 KB**. Die Varianten stammen aus demselben deterministischen Skript wie die README-Bilder. Die Ironie ist erwähnenswert: Das einzige externe Skript der Seite ist genau das, was uns entdecken ließ, dass sie langsam war.
...
- **Anmeldecodes und Feedback-Benachrichtigungen gehen jetzt von einer verifizierten Domain aus** (`noreply@arts44.dev` — SPF, DKIM, MX und DMARC seit dem 13.08.2026 eingerichtet). Eine brandneue Domain hat keinerlei Versandreputation, und die ersten Messungen sagen das unverblümt: binnen einer Stunde wurde eine Nachricht an Proximus **abgewiesen** (`554 … poor reputation of a domain used in message transfer`, als vorübergehend markiert), und eine Nachricht an Gmail **landete im Spam-Ordner**. Das ist das erwartete Verhalten einer Domain ohne Historie — es bessert sich mit dem Volumen — doch solange die Reputation wächst, muss ein Code womöglich aus dem Spam gefischt werden, oder ein Anbieter weist ihn ganz ab. Ein fehlgeschlagener Versand blockiert weiterhin nichts: Die Rückmeldung wird ohnehin in der Datenbank gespeichert.
