...
## ▶️ **[Probeer live → arts44.dev](https://arts44.dev/)**
...
Elke demo komt uit `capture_demos.py`, met dezelfde deterministische seed als de stilstaande beelden, die tussen twee runs **geen zichtbaar verschil** vertonen: elke animatie wordt vóór elke opname op een gekozen fase bevroren. De drempel is becijferd en met één commando te controleren (`compare_captures.py`); byte-identiek is niet haalbaar — er blijft antialiasing-ruis van enkele tientallen pixels op 10 niveaus van 255. De GIF's draaien op 33,3 fps; de 60 fps-versie staat ernaast: [snel toevoegen](screenshots/demo-quick-add.mp4) · [navigatie](screenshots/demo-nav.mp4) · [badges](screenshots/demo-badges.mp4) · [seizoenen](screenshots/demo-seasons.mp4).
...
| Tests | **Ingebouwde testrunner van Node** (`node --test`) — 955 tests, geen testframework |
...
**Oplossing:** de logica is zo gefactoriseerd dat ze browservrij is en wordt gedekt door **955 tests op de ingebouwde runner van Node** — geen testafhankelijkheden, geen echt netwerk. De CI herbouwt bovendien de bundle en faalt als het gecommitte artefact verouderd is.

---

### Wat de tests dekken — en wat niet

955 tests op Node's ingebouwde runner, zonder framework. Precies zijn over de grens telt zwaarder dan het aantal:
...
npm test        # 955 tests, node --test, zonder framework
```
...
- **De startpagina draagt een cookieloze bezoekersteller; de app laadt niets.** De etalagepagina op de domeinroot bevat Cloudflare Web Analytics — geen cookies, geen cross-site tracking, alleen bezoekaantallen. De applicatie (`/app/`) laadt nog steeds **nul externe bronnen**, en een testsuite breekt de build zodra er één opduikt. De enige toegestane uitzondering is met naam en toenaam vastgelegd in dezelfde test.
- **De startpagina heeft een gemeten gewichtsbudget — 120 KB, bewaakt door een test.** Cloudflares eigen bezoekersteller onthulde de traagheid: de schermafbeeldingen waren het LCP-element op 5 091 ms bij echt P90-verkeer, 89 % van het paginagewicht. Opgelost met WebP-varianten op hun werkelijke weergavegrootte, `fetchpriority` op de LCP en lazy loading op de afbeelding die onder 720 px verborgen is — gemeten, niet afgeleid: mobiel van 277 KB naar **73 KB**, desktop naar **99 KB**. De varianten komen uit hetzelfde deterministische script als de README-afbeeldingen. De ironie is het noteren waard: het enige externe script van de pagina is precies wat ons liet ontdekken dat hij traag was.
...
- **Inlogcodes en feedbackmeldingen vertrekken voortaan vanaf een geverifieerd domein** (`noreply@arts44.dev` — SPF, DKIM, MX en DMARC actief sinds 13-08-2026). Een gloednieuw domein heeft geen verzendreputatie, en de eerste metingen zeggen dat onomwonden: binnen het uur werd een bericht aan Proximus **geweigerd** (`554 … poor reputation of a domain used in message transfer`, als tijdelijk gemarkeerd) en kwam een bericht aan Gmail **in de spammap** terecht. Dat is het verwachte gedrag van een domein zonder historie — het verbetert met het volume — maar zolang de reputatie wordt opgebouwd, moet een code soms uit de ongewenste post worden gevist, of weigert een provider hem helemaal. Een mislukte verzending blokkeert nog steeds niets: de feedback wordt hoe dan ook in de database bewaard.
