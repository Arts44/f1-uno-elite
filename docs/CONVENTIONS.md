...
- **UN CONTEXTE « VIERGE » AU SENS DES DONNÉES N'EST PAS UN PREMIER LANCEMENT.** Deux fois payé en deux semaines, sur deux sujets sans rapport : ① les six défauts d'affichage de la page Stats vivaient tous dans l'état « 1 carte » — quarante captures existaient, toutes prises sur le seed à 72 cartes, si bien que l'écran d'un nouvel utilisateur n'était **jamais rendu** ; ② une fiche d'échange reçue par lien était perdue au premier lancement — or un lien partagé vise **en priorité** un appareil qui n'a jamais vu l'app, donc la fonctionnalité échouait exactement pour son public principal. Dans les deux cas la vérification existait et passait au vert : elle partait d'un `localStorage` effacé, ce qui **saute** le choix de langue, la mise en route et la proposition de tutoriel — c'est-à-dire précisément la séquence qui casse. **Règle : toute fonctionnalité atteignable par un lien externe (`#backup=`, `#trade=`, retour d'authentification) se vérifie depuis un profil qui n'a JAMAIS vu l'app, parcours de mise en route joué en entier.** Le harnais le porte désormais : `seed_jamais_lancee()` (qui renvoie le vide — le contexte doit être créé SANS `add_init_script`) et `franchir_premier_lancement(page)` dans `capture_seed.py`, qui joue les étapes dans l'ordre et renvoie celles qui ont été franchies, pour qu'un test l'affirme au lieu de le supposer. Corollaire : l'état d'un nouvel utilisateur mérite une capture au même titre que l'état plein — voir `seed_presque_vide(n)`.
- **SOUPÇONNER L'INSTRUMENT AVANT LE PRODUIT.** C'est UNE règle, pas une collection d'anecdotes : quand une mesure accuse le code, elle énonce une hypothèse, pas un verdict. Avant de corriger quoi que ce soit, faire rendre à l'instrument le cas RÉEL et vérifier qu'il sait le mesurer. **Ordre de suspicion : l'instrument, puis la chaîne (bundle reconstruit ? cache purgé ? bon fichier ?), puis le contrôle, et le produit en DERNIER.** La règle a cinq façons connues de se manifester — les reconnaître fait gagner l'exécution qui les démasque, mais c'est la règle qu'on retient, pas la liste : ① **la condition de rendu n'est pas celle du terrain** (un banc de lisibilité à densité 3 « prouvait » qu'aucun trait fin ne survit à 14 px — faux à densité 1, la densité réelle) ; ② **l'API répond à côté de la question** (`getBBox()` sur un `<use>` rend 0…120 sur un tracé qui n'occupe pas son cadre — mesurer au raster) ; ③ **la donnée de test est dégénérée** (un motif répétitif, `'A'`×700, trompe un détecteur qui marche très bien sur une charge réelle) ; ④ **on mesure un état de repos alors que le défaut vit dans la TRANSITION** (« rien n'est recouvert » était exact en fin de défilement et faux pendant — voir l'entrée dédiée ci-dessous) ; ⑤ **on lit la sortie d'une commande sans lire son code de sortie** (un `grep` ou un `| tail` renvoie 0 et laisse passer un échec — voir l'entrée dédiée). **Le coût d'un contrôle d'instrument est une exécution ; le coût de l'omettre est une correction inventée sur un défaut qui n'existe pas — ou, pire, un vrai défaut masqué par un banc trop indulgent.**
