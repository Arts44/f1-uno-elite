...
- **Le bundle IIFE MASQUE les symboles oubliés lors d'un déplacement de code — seul `index-dev.html` les révèle.** Vécu pendant le découpage de `badges.js` : trois symboles (`DAY_MS`, `autoBadgeUnlocked`, `_bt`) sont restés dans le module d'origine alors que le code qui les utilise avait déménagé. `npm run build` passait, `npm test` passait, **953 tests verts** — parce qu'esbuild en `--format=iife` met tout dans une portée unique, où un symbole « manquant » reste visible. En modules bruts (`index-dev.html`), c'est un `ReferenceError` immédiat. **Après tout déplacement de code entre modules, ouvrir `index-dev.html` et exercer le chemin déplacé.** Un test qui n'emprunte pas ce chemin ne prouve rien : celui du rythme (jours consécutifs) n'était couvert par aucun test, et c'est la vérification navigateur d'un AUTRE pas qui l'a fait tomber.
