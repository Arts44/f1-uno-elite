...
- **SOUPÇONNER L'INSTRUMENT AVANT LE PRODUIT — la règle générale, dont tout le reste de cette section n'est qu'une collection de cas.** Un banc qui accuse le code est une hypothèse, pas un verdict : avant de corriger quoi que ce soit, faire rendre à l'instrument le cas RÉEL et vérifier qu'il sait le mesurer. Quatre fois payé, sur quatre sujets sans rapport : ① un banc de lisibilité à densité 3 « prouvait » qu'aucun trait fin ne survit à 14 px — faux à densité 1, la densité réelle ; ② `getBBox()` sur un `<use>` rendait 0…120 sur un tracé qui n'occupe pas son cadre ; ③ un contrôle négatif passait au vert parce que le bundle n'était pas reconstruit ; ④ un décodeur QR échouait sur une charge dégénérée (`'A'`×700) et sur une capture plus large que la fenêtre — deux artefacts de banc, aucun défaut du produit. **Le coût d'un contrôle d'instrument est une exécution ; le coût de l'omettre est une correction inventée sur un défaut qui n'existe pas** — ou, pire, un vrai défaut masqué par un banc trop indulgent. Ordre : l'instrument, la chaîne (bundle, cache, bon fichier), le contrôle, et le produit en dernier.
