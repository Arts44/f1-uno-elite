...
export const CHANGELOG = [
  ...
  },
  {
    ...
    date: '2026-08-19',
    changes: {
      en: ['The bottom of the screen has an owner. Update notices, the “what’s new” notice, the install prompt, badge pop-ups and the review invitation used to be placed by six different rules — they overlapped each other, covered the navigation bar, and two of them could land in exactly the same spot, one hiding the other completely. They now share a single anchored area, only one shows at a time, and nothing is lost: a badge pop-up briefly takes priority because it fades on its own, and the notice it interrupted comes straight back. The buttons meet the 44 px touch target and the “Reload” button now has readable contrast in dark mode. Visible change: the small confirmation toast (undo, errors) has moved from the bottom-right corner into that same area — it no longer covers the navigation bar. A “what’s new” notice you never actually saw is no longer counted as read: it now waits until an update check has finished, so it can’t flash for a moment and vanish, and it comes back next time if you never got to it.'],
      fr: ['Le bas de l’écran a un propriétaire. Le bandeau de mise à jour, celui des nouveautés, la proposition d’installation, les annonces de badge et l’invitation à donner un avis étaient placés par six règles différentes — ils se chevauchaient, recouvraient la barre de navigation, et deux d’entre eux pouvaient tomber exactement au même endroit, l’un cachant l’autre entièrement. Ils partagent maintenant une seule zone amarrée, un seul s’affiche à la fois, et rien ne se perd : une annonce de badge passe brièvement devant parce qu’elle s’efface toute seule, et le bandeau qu’elle a interrompu revient aussitôt. Les boutons respectent les 44 px de zone tactile et le bouton « Recharger » a enfin un contraste lisible en thème sombre. Changement visible : le petit message de confirmation (annuler, erreurs) quitte le coin bas-droit pour cette même zone — il ne recouvre plus la barre de navigation. Un message « nouveautés » que vous n’avez pas vu n’est plus compté comme lu : il attend la fin de la vérification de mise à jour, il ne peut donc plus s’afficher un instant puis disparaître, et il revient la fois suivante si vous n’avez pas eu le temps de le lire.'],
      es: ['La parte inferior de la pantalla tiene dueño. El aviso de actualización, el de novedades, la propuesta de instalación, los anuncios de insignia y la invitación a dejar una opinión se colocaban con seis reglas distintas — se solapaban, tapaban la barra de navegación, y dos de ellos podían caer exactamente en el mismo sitio, ocultándose por completo. Ahora comparten una única zona anclada, solo se muestra uno a la vez, y nada se pierde: un anuncio de insignia pasa brevemente delante porque desaparece solo, y el aviso que interrumpió vuelve enseguida. Los botones respetan los 44 px de zona táctil y el botón «Recargar» por fin tiene contraste legible en tema oscuro. Cambio visible: el pequeño mensaje de confirmación (deshacer, errores) deja la esquina inferior derecha por esa misma zona — ya no tapa la barra de navegación. Un aviso de «novedades» que no ha llegado a ver ya no se cuenta como leído: ahora espera a que termine la comprobación de actualización, así que no puede aparecer un instante y desaparecer, y vuelve la próxima vez si no le dio tiempo.'],
      zh: ['屏幕底部终于有了归属。更新提示、更新内容提示、安装建议、徽章通知和评价邀请此前由六套不同的规则摆放——它们互相重叠、遮住导航栏，其中两个甚至会落在完全相同的位置，一个把另一个整个盖住。现在它们共用一个统一锚定的区域，同一时间只显示一个，而且不会丢失任何消息：徽章通知会短暂插队，因为它自己会消失，被它打断的提示随后立刻回来。按钮达到 44 px 的可点击尺寸，「重新加载」按钮在深色主题下终于有了可读的对比度。可见的变化：小的确认提示（撤销、错误）从右下角移入同一区域——不再遮挡导航栏。 你没真正看到的「更新内容」提示不再算作已读：它会等更新检查结束后再出现，因此不会一闪而过，如果你没来得及看，下次还会再来。'],
      it: ['La parte bassa dello schermo ha un proprietario. L’avviso di aggiornamento, quello delle novità, la proposta di installazione, gli annunci di distintivo e l’invito a lasciare un parere erano collocati da sei regole diverse — si sovrapponevano, coprivano la barra di navigazione, e due di essi potevano finire esattamente nello stesso punto, nascondendosi del tutto a vicenda. Ora condividono un’unica zona ancorata, se ne mostra uno alla volta, e nulla va perduto: un annuncio di distintivo passa brevemente davanti perché svanisce da solo, e l’avviso che ha interrotto torna subito. I pulsanti rispettano i 44 px di area tattile e il pulsante «Ricarica» ha finalmente un contrasto leggibile nel tema scuro. Cambiamento visibile: il piccolo messaggio di conferma (annulla, errori) lascia l’angolo in basso a destra per la stessa zona — non copre più la barra di navigazione. Un avviso «novità» che non avete visto non viene più contato come letto: ora attende la fine del controllo di aggiornamento, quindi non può comparire un istante e sparire, e torna la volta successiva se non avete fatto in tempo.'],
      nl: ['De onderkant van het scherm heeft een eigenaar. De updatemelding, de melding met nieuwtjes, het installatievoorstel, de badgemeldingen en de uitnodiging om een mening te geven werden door zes verschillende regels geplaatst — ze overlapten elkaar, bedekten de navigatiebalk, en twee ervan konden precies op dezelfde plek belanden, waarbij de ene de andere volledig verborg. Ze delen nu één verankerde zone, er wordt er maar één tegelijk getoond, en er gaat niets verloren: een badgemelding gaat even voor omdat ze vanzelf verdwijnt, en de melding die ze onderbrak komt meteen terug. De knoppen halen de 44 px aanraakzone en de knop «Herladen» heeft eindelijk leesbaar contrast in het donkere thema. Zichtbare verandering: het kleine bevestigingsbericht (ongedaan maken, fouten) verlaat de rechteronderhoek voor diezelfde zone — het bedekt de navigatiebalk niet meer. Een «wat is nieuw»-melding die u niet echt gezien hebt, telt niet langer als gelezen: ze wacht nu tot de updatecontrole klaar is, kan dus niet even opflitsen en verdwijnen, en komt de volgende keer terug als u er niet aan toe kwam.'],
      de: ['Der untere Bildschirmrand hat einen Eigentümer. Der Update-Hinweis, der Neuigkeiten-Hinweis, der Installationsvorschlag, die Abzeichen-Meldungen und die Einladung zu einer Rückmeldung wurden von sechs verschiedenen Regeln platziert — sie überlappten sich, verdeckten die Navigationsleiste, und zwei davon konnten an exakt derselben Stelle landen, wobei einer den anderen vollständig verbarg. Sie teilen sich nun einen einzigen verankerten Bereich, es wird immer nur einer gezeigt, und nichts geht verloren: eine Abzeichen-Meldung geht kurz vor, weil sie von selbst verschwindet, und der unterbrochene Hinweis kommt sofort zurück. Die Schaltflächen erreichen die 44-px-Tippfläche, und die Schaltfläche „Neu laden“ hat im dunklen Thema endlich lesbaren Kontrast. Sichtbare Änderung: die kleine Bestätigungsmeldung (rückgängig, Fehler) verlässt die untere rechte Ecke und zieht in denselben Bereich — sie verdeckt die Navigationsleiste nicht mehr. Ein Neuigkeiten-Hinweis, den Sie gar nicht gesehen haben, gilt nicht mehr als gelesen: er wartet jetzt das Ende der Update-Prüfung ab, kann also nicht kurz aufblitzen und verschwinden, und kommt beim nächsten Mal wieder, wenn Sie nicht dazu kamen.'],
    },
  },
  {
    version: '1.76.0',
  ...
  },
  {
    ...
    date: '2026-08-18',
    changes: {
      en: ['A trade sheet you receive is now kept. Before, opening a shared link on a device that had never run the app lost the sheet: first-run setup and the guided tour covered it and it was gone — exactly the case a shared link is meant for. It is now saved on arrival, appears as its own entry in Stats with its expiry written out, can be reopened or deleted, and is replaced only after a confirmation that names the one being replaced. One sheet at a time, kept 30 days, and never included in backups or cloud sync — it is someone else\'s collection.'],
      fr: ['Une fiche d’échange reçue est désormais conservée. Avant, ouvrir un lien partagé sur un appareil qui n’avait jamais lancé l’app perdait la fiche : la mise en route et la visite guidée passaient par-dessus et elle disparaissait — exactement le cas pour lequel un lien partagé existe. Elle est maintenant enregistrée à l’arrivée, apparaît dans les Stats avec son échéance écrite, se rouvre ou s’efface, et n’est remplacée qu’après une confirmation qui nomme celle qui part. Une fiche à la fois, gardée 30 jours, et jamais incluse dans les sauvegardes ni le cloud — c’est la collection de quelqu’un d’autre.'],
      es: ['Una ficha de cambio recibida ahora se conserva. Antes, abrir un enlace compartido en un dispositivo que nunca había ejecutado la app perdía la ficha: la puesta en marcha y la visita guiada pasaban por encima y desaparecía — justo el caso para el que existe un enlace compartido. Ahora se guarda al llegar, aparece en Estadísticas con su caducidad escrita, se puede reabrir o borrar, y solo se reemplaza tras una confirmación que nombra la que se va. Una ficha a la vez, guardada 30 días, y nunca incluida en copias de seguridad ni en la nube — es la colección de otra persona.'],
      zh: ['收到的交换单现在会被保存。此前，在从未启动过应用的设备上打开分享链接会丢失交换单：初次设置与引导教程会覆盖它，它就此消失——而这恰恰是分享链接存在的场景。现在它在到达时即被保存，在统计页中单独显示并写明有效期，可重新打开或删除，只有在确认（并明确说明将被替换的是哪一份）之后才会被替换。同一时间保留一份，保存 30 天，且绝不写入备份或云同步——那是别人的收藏。'],
      it: ['Una scheda di scambio ricevuta viene ora conservata. Prima, aprire un link condiviso su un dispositivo che non aveva mai avviato l’app perdeva la scheda: avvio iniziale e visita guidata le passavano sopra e spariva — esattamente il caso per cui esiste un link condiviso. Ora viene salvata all’arrivo, compare nelle Statistiche con la scadenza scritta, si riapre o si elimina, e viene sostituita solo dopo una conferma che nomina quella che se ne va. Una scheda alla volta, tenuta 30 giorni, e mai inclusa nei backup né nel cloud — è la collezione di qualcun altro.'],
      nl: ['Een ontvangen ruilblad wordt nu bewaard. Voorheen ging het verloren wanneer je een gedeelde link opende op een apparaat dat de app nooit had gedraaid: het opstartproces en de rondleiding kwamen eroverheen en het was weg — precies het geval waarvoor een gedeelde link bestaat. Het wordt nu bij aankomst opgeslagen, verschijnt in Statistieken met de vervaldatum erbij, kan opnieuw worden geopend of verwijderd, en wordt pas vervangen na een bevestiging die het vertrekkende blad benoemt. Eén blad tegelijk, 30 dagen bewaard, en nooit opgenomen in back-ups of cloudsynchronisatie — het is de collectie van iemand anders.'],
      de: ['Ein erhaltenes Tauschblatt wird jetzt aufbewahrt. Zuvor ging es verloren, wenn man einen geteilten Link auf einem Gerät öffnete, das die App nie gestartet hatte: Einrichtung und geführte Tour legten sich darüber und es war weg — genau der Fall, für den ein geteilter Link existiert. Es wird jetzt bei Ankunft gespeichert, erscheint in den Statistiken mit ausgeschriebener Frist, lässt sich erneut öffnen oder löschen, und wird erst nach einer Bestätigung ersetzt, die das weichende Blatt benennt. Ein Blatt zur Zeit, 30 Tage aufbewahrt, und nie in Backups oder Cloud-Sync enthalten — es ist die Sammlung von jemand anderem.'],
    },
  },
  {
    version: '1.73.0',
  ...
  },
  {
    ...
    date: '2026-08-18',
    changes: {
      en: ['Six display fixes on the Stats page, all in the state a brand-new collector sees. Labels now agree in number in every language — no more “1 cards”. The donut label no longer spills out of its ring, a single-rarity ring shows that rarity’s own colour, counts are no longer written twice, the floating bar fades over the content instead of cutting it, and “featured cards” stays hidden until it has something to say.'],
      fr: ['Six corrections d’affichage sur la page Stats, toutes dans l’état que voit un tout nouveau collectionneur. Les libellés s’accordent désormais en nombre dans toutes les langues — fini les « 1 cartes ». Le libellé du donut ne déborde plus de son anneau, un anneau d’une seule rareté prend la couleur de cette rareté, les nombres ne sont plus écrits deux fois, la barre flottante fond sur le contenu au lieu de le couper, et « cartes phares » reste masqué tant qu’il n’a rien à dire.'],
      es: ['Seis correcciones de visualización en la página de estadísticas, todas en el estado que ve un coleccionista recién llegado. Las etiquetas concuerdan ahora en número en todos los idiomas — se acabó el « 1 cartas ». La etiqueta del donut ya no se sale de su anillo, un anillo de una sola rareza toma el color de esa rareza, las cifras ya no se escriben dos veces, la barra flotante se difumina sobre el contenido en vez de cortarlo, y « cartas destacadas » permanece oculto hasta tener algo que decir.'],
      zh: ['统计页的六处显示修复，全部集中在新收藏者初次看到的状态。各语言的标签现在都能正确处理单复数——不会再出现「1 cards」。圆环图的文字不再溢出环内，单一稀有度的圆环会采用该稀有度自身的颜色，数字不再重复出现，悬浮栏改为渐隐覆盖内容而非直接截断，「精选卡片」在没有内容可讲时保持隐藏。'],
      it: ['Sei correzioni di visualizzazione nella pagina Statistiche, tutte nello stato che vede un collezionista appena arrivato. Le etichette concordano ora nel numero in tutte le lingue — basta con « 1 carte ». L’etichetta del donut non esce più dal suo anello, un anello di una sola rarità prende il colore di quella rarità, i numeri non sono più scritti due volte, la barra flottante sfuma sul contenuto invece di tagliarlo, e « carte in evidenza » resta nascosto finché non ha qualcosa da dire.'],
      nl: ['Zes weergavefixes op de Statistieken-pagina, allemaal in de staat die een gloednieuwe verzamelaar ziet. Labels komen nu in elk taalgebruik in getal overeen — geen « 1 kaarten » meer. Het donutlabel loopt niet meer buiten zijn ring, een ring met één zeldzaamheid krijgt de kleur van die zeldzaamheid, getallen staan er niet meer dubbel, de zwevende balk vervaagt over de inhoud in plaats van hem af te snijden, en « uitgelichte kaarten » blijft verborgen tot het iets te zeggen heeft.'],
      de: ['Sechs Anzeigekorrekturen auf der Statistikseite, alle in dem Zustand, den ein ganz neuer Sammler sieht. Beschriftungen stimmen jetzt in jeder Sprache im Numerus — kein « 1 Karten » mehr. Die Donut-Beschriftung läuft nicht mehr aus ihrem Ring, ein Ring mit nur einer Seltenheit trägt deren eigene Farbe, Zahlen stehen nicht mehr doppelt, die schwebende Leiste blendet über den Inhalt statt ihn abzuschneiden, und « Vorzeigekarten » bleibt verborgen, bis es etwas zu sagen hat.'],
    },
  },
  {
    version: '1.71.0',
    date: '2026-08-18',
    changes: {
      en: ['IMPORTANT — your backup QR code was unusable, and that could have cost you your collection. Measured: at its old fixed size, the backup QR fell below the readable density in EVERY size it could take — beyond a very small collection, no camera could read it. A backup you cannot restore is not a backup. Both QR codes (backup and trade sheet) now scale with their data, so a scan works the first time. Verified with an independent QR reader, not with our own encoder.'],
      fr: ['IMPORTANT — votre QR de sauvegarde était inutilisable, et cela pouvait vous coûter votre collection. Mesuré : à son ancienne taille fixe, le QR de sauvegarde tombait sous la densité lisible dans TOUTES les tailles qu’il pouvait prendre — au-delà d’une toute petite collection, aucune caméra ne pouvait le lire. Une sauvegarde qu’on ne peut pas restaurer n’est pas une sauvegarde. Les deux QR (sauvegarde et fiche d’échange) suivent désormais la taille de leurs données, et un scan fonctionne du premier coup. Vérifié avec un lecteur de QR indépendant, pas avec notre propre encodeur.'],
      es: ['IMPORTANTE — tu código QR de copia de seguridad era inutilizable, y eso podía costarte tu colección. Medido: a su antiguo tamaño fijo, el QR de copia caía por debajo de la densidad legible en TODOS los tamaños que podía tomar — más allá de una colección muy pequeña, ninguna cámara podía leerlo. Una copia que no puedes restaurar no es una copia. Ambos QR (copia y ficha de cambio) siguen ahora el tamaño de sus datos, y un escaneo funciona a la primera. Verificado con un lector de QR independiente, no con nuestro propio codificador.'],
      zh: ['重要 — 您的备份二维码此前无法使用，这可能让您失去整个收藏。经测量：在旧的固定尺寸下，备份二维码在它可能取到的所有尺寸上都低于可识别密度——只要收藏不是极小，任何相机都读不出来。无法恢复的备份不是备份。现在两种二维码（备份与交换单）都会随数据量调整尺寸，扫码一次即可成功。此修复由独立的二维码识别库验证，而非我们自己的编码器。'],
      it: ['IMPORTANTE — il tuo codice QR di backup era inutilizzabile, e questo poteva costarti la collezione. Misurato: alla sua vecchia dimensione fissa, il QR di backup scendeva sotto la densità leggibile in TUTTE le dimensioni che poteva assumere — oltre una collezione molto piccola, nessuna fotocamera riusciva a leggerlo. Un backup che non si può ripristinare non è un backup. Entrambi i QR (backup e scheda di scambio) seguono ora la dimensione dei loro dati, e una scansione funziona al primo colpo. Verificato con un lettore QR indipendente, non con il nostro codificatore.'],
      nl: ['BELANGRIJK — je back-up-QR-code was onbruikbaar, en dat had je je collectie kunnen kosten. Gemeten: op zijn oude vaste formaat zakte de back-up-QR onder de leesbare dichtheid in ELK formaat dat hij kon aannemen — bij meer dan een heel kleine collectie kon geen enkele camera hem lezen. Een back-up die je niet kunt terugzetten is geen back-up. Beide QR-codes (back-up en ruilblad) volgen nu de omvang van hun data, en scannen werkt in één keer. Gecontroleerd met een onafhankelijke QR-lezer, niet met onze eigen encoder.'],
      de: ['WICHTIG — dein Backup-QR-Code war unbrauchbar, und das hätte dich deine Sammlung kosten können. Gemessen: in seiner alten festen Größe fiel der Backup-QR in JEDER Größe, die er annehmen konnte, unter die lesbare Dichte — jenseits einer sehr kleinen Sammlung konnte ihn keine Kamera lesen. Ein Backup, das man nicht wiederherstellen kann, ist kein Backup. Beide QR-Codes (Backup und Tauschblatt) folgen jetzt der Größe ihrer Daten, und ein Scan klappt beim ersten Versuch. Geprüft mit einem unabhängigen QR-Leser, nicht mit unserem eigenen Encoder.'],
    },
  },
  {
    version: '1.70.0',
