...
| Tests | **Node's built-in test runner** (`node --test`) — 955 tests, no test framework |
...
**Solution:** the logic was factored to be browser-free and is covered by **955 tests on Node's built-in runner** — no test dependencies, no real network. CI also rebuilds the bundle and fails if the committed artifact is stale.
...
- **The visual freeze is reproducible.** Every still capture in this README is regenerated by a deterministic in-repo script — seed computed from the card data, honesty-checked, and **free of any visible difference between runs**: every animation is frozen at a chosen phase before each shot, so two regenerations of an unchanged app differ only by antialiasing noise — a few dozen pixels at 10 levels out of 255. Byte-identity was claimed three times and refuted three times; what replaces it is checkable in one command (`compare_captures.py`), which fails on any difference above 16/255 or 0.4 % of pixels. A capture diff is a signal, not noise. — a badge may not appear unlocked unless its condition truly holds. The four animated demos are generated the same way, by `capture_demos.py`, which also checks frame by frame that the 280 ms nav-bead glide is sampled above 30 fps.
- **955 tests on vanilla JS, no test framework** — Node's built-in runner over the browser-free logic: storage migrations, rarity ladder, badge conditions and difficulty model, cloud failure scenarios with stubbed `fetch`, backup codes, i18n coverage. Stat and rarity palettes are contrast-checked by hand in both themes when they change; no automated scan is committed here.

### What the tests cover — and what they don't

955 tests on Node's built-in runner, no test framework. Being precise about the boundary matters more than the number:
...
npm test        # 955 tests, node --test, no framework
```
...
- **The home page carries a cookieless audience counter; the app loads nothing.** The landing page at the domain root includes Cloudflare Web Analytics — no cookies, no cross-site tracking, just visit counts. The application itself (`/app/`) still loads **zero external resources**, and a test suite fails the build if a single one appears. The one allowed exception is named, file by file, in the same test.
- **The landing page has a measured weight budget — 120 KB, enforced by a test.** Cloudflare's own analytics beacon revealed the page was slow: its screenshots were the LCP element at 5,091 ms on real P90 traffic, 89 % of the page weight. Fixed with WebP variants sized for their actual display, `fetchpriority` on the LCP and lazy loading on the shot hidden below 720 px — measured, not deduced: mobile went from 277 KB to **73 KB**, desktop to **99 KB**. The variants come out of the same deterministic capture script as the README shots, byte-identical across runs. The irony is worth recording: the page's only external script is precisely what allowed us to discover it was slow.
...
- **Sign-in codes and feedback notifications now leave a verified domain** (`noreply@arts44.dev` — SPF, DKIM, MX and DMARC in place since 2026-08-13). A brand-new domain has no sending reputation, and the first measurements say so plainly: within the hour, a message to Proximus was **rejected outright** (`554 … poor reputation of a domain used in message transfer`, flagged transient) and a message to Gmail **arrived in the spam folder**. That is the expected behaviour of a domain with no history — it improves as volume builds — but while reputation is being earned, a code may need to be fished out of the junk folder, and some ISPs may refuse it altogether. Sending errors still never block anything: feedback is stored in the database either way.
