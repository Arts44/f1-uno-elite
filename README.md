...
- **The Codacy grade is B, and stays B on purpose.** Three of the four quality goals are green: 0 open issues, 4 % duplication, and 62.72 % coverage against a 60 % target. The fourth is not — 13 of the 30 analysed source files sit above the complexity threshold. That threshold is adjustable, and raising it would turn the number green without changing a line of code; it hasn't been raised, because four of those files genuinely do too much. The two heaviest are `badges.js` and `cloud.js`. Splitting them is the next piece of work — it is what stands between the project and the trade-list export, and between it and multi-season support. One caveat on the metric itself: it counts *files*, so it penalises an architecture built from a few large modules — the same code spread over 300 files would pass without a single line changing. That is a fact about the measurement, not an excuse: the four dense modules are genuinely dense, and they are the ones the refactor targets.
