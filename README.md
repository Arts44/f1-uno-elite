...
- **The visual freeze is reproducible.** Every still capture in this README is regenerated by an in-repo script whose **seed** is deterministic — computed from the card data and honesty-checked (the pixels are not byte-identical between runs: unfrozen animations land on a slightly different frame, SSIM 0.999988) — a badge may not appear unlocked unless its condition truly holds. The four animated demos are generated the same way, by `capture_demos.py`, which also checks frame by frame that the 280 ms nav-bead glide is sampled above 30 fps.
...
- **The Codacy grade is A — and one of its four quality goals is red.** Measured on commit `e19d1fa`, 121 files, 9 741 lines. Green: duplication at 6 % (goal 10 %), and coverage at 69 % — Codacy's gauge and the local `npm run test:cov` agree to a tenth this time (69.10 %), against a 60 % goal, so nine points of margin instead of the 2.7 it used to be. Red: complexity, with **44 % of analysed files above the threshold** against a 10 % goal — the ratio went *up* after `cloud.js` was split into four, which says something about the metric: it counts *files*, so a codebase of a few large modules is penalised while the same code spread over 300 files would pass without a line changing. That is a fact about the measurement, not an excuse; `badges.js` genuinely does too much. And a fourth number the badge hides: **11 open issues**, of which **8 are in the capture scripts added for this README** (`subprocess` and `exec` in `capture_demos.py` — dev tooling, not shipped code), 2 unused variables in `account.js`, and 1 hard-coded-password false positive in `cloud-auth.js`. The badge above is real; it is not the whole picture, which is why the whole picture is here.
