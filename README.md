...
| Tests | **Node's built-in test runner** (`node --test`) — 623 tests, no test framework |
...
**Solution:** the logic was factored to be browser-free and is covered by **623 tests on Node's built-in runner** — no test dependencies, no real network. CI also rebuilds the bundle and fails if the committed artifact is stale.
...
- **623 tests on vanilla JS, no test framework** — Node's built-in runner over the browser-free logic: storage migrations, rarity ladder, badge conditions and difficulty model, cloud failure scenarios with stubbed `fetch`, backup codes, i18n coverage. A WCAG-AA contrast scan runs against both themes.

### What the tests cover — and what they don't

623 tests on Node's built-in runner, no test framework. Being precise about the boundary matters more than the number:
...
npm test        # 623 tests, node --test, no framework
```
...
- **The Codacy grade is A — and two of its four quality goals are red.** Green: 0 open issues, 4 % duplication. Red: complexity, with 13 of the 30 analysed source files above the threshold (43 %, against a 10 % goal); and coverage, which is measured locally at 62.72 % but is not reported to Codacy, so the gauge reads as unmeasured. The badge above is real, but it is not the whole picture, which is why the whole picture is here. The complexity threshold is adjustable and raising it would turn that gauge green without changing a line of code; it hasn't been raised, because four of those files genuinely do too much — the two heaviest are `badges.js` and `cloud.js`, and splitting them is the next piece of work, since it is what stands between the project and the trade-list export, and between it and multi-season support. One caveat on the metric itself: it counts *files*, so it penalises an architecture built from a few large modules — the same code spread over 300 files would pass without a single line changing. That is a fact about the measurement, not an excuse.
