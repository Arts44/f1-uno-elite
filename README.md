...
[![Codacy Badge](https://api.codacy.com/project/badge/Grade/4e7d9096628044aba780a0b2000aee8c)](https://app.codacy.com/gh/Arts44/f1-uno-elite?utm_source=github.com&utm_medium=referral&utm_content=Arts44/f1-uno-elite&utm_campaign=Badge_Grade)
...
- 🎓 **Interactive tutorial, 28 steps in 5 chapters** — one chapter per page, in tab order. A guided tour where you *perform* the real actions, running in a sandbox that reverts every change when it ends.
...
| Tests | **Node's built-in test runner** (`node --test`) — 519 tests, no test framework |
...
**Solution:** the logic was factored to be browser-free and is covered by **519 tests on Node's built-in runner** — no test dependencies, no real network. CI also rebuilds the bundle and fails if the committed artifact is stale.
...
- **Zero layout shift, measured — not assumed.** Grid tiles are verified pixel-identical across visual releases (reference row: 246.59 px), the loading skeleton adds no height, and every visual pass ships with a before/after re-measure in the commit message.
- **Performance fixed by measurement.** The quick-add path was profiled (CDP, CPU×6): one tap used to trigger up to five full save+badge-evaluation passes and a 101-tile rebuild — batched writes and targeted tile updates brought it from ~300 ms to ~45 ms on a mid-range phone profile.
...
- **The visual freeze is reproducible.** Every still capture in this README is regenerated by a deterministic in-repo script whose seed is computed from the card data and honesty-checked — a badge may not appear unlocked unless its condition truly holds. The three animated demos are recorded by hand.
- **519 tests on vanilla JS, no test framework** — Node's built-in runner over the browser-free logic: storage migrations, rarity ladder, badge conditions and difficulty model, cloud failure scenarios with stubbed `fetch`, backup codes, i18n coverage. A WCAG-AA contrast scan runs against both themes.

### What the tests cover — and what they don't

519 tests on Node's built-in runner, no test framework. Being precise about the boundary matters more than the number:

- **Covered:** storage migrations and the season-scoped key scheme; the seven-level rarity ladder including complete-set promotion; all badge conditions and the difficulty model; collector lists (missing, doubles, trade); backup encode/decode round-trips; cloud helpers against a stubbed `fetch`, including failure paths; i18n key parity across the 7 languages and duplicate-key detection; the service-worker precache against the real import graph; markup contracts for keyboard access; provenance of every externally-fed `innerHTML`; WCAG-AA contrast across both themes.
- **Not covered:** actual rendering (no DOM assertions beyond markup strings), the service worker's runtime fetch behaviour, real network calls, IndexedDB, install prompts, and anything requiring a browser engine — those are verified by hand and by the deterministic screenshot pass, not by the suite. Coverage percentage is deliberately not published: it would measure the browser-free slice and read as if it measured the app.
...
npm test        # 519 tests, node --test, no framework
```
...
- **The Codacy grade is B, and stays B on purpose.** Of the four quality goals Codacy tracks, two are green — 0 open issues and 4 % duplication — and line coverage sits at 62.72 % against a 60 % target, measured by the test suite itself. The fourth is not — 13 of the 30 analysed source files sit above the complexity threshold. That threshold is adjustable, and raising it would turn the number green without changing a line of code; it hasn't been raised, because four of those files genuinely do too much. The two heaviest are `badges.js` and `cloud.js`. Splitting them is the next piece of work — it is what stands between the project and the trade-list export, and between it and multi-season support. One caveat on the metric itself: it counts *files*, so it penalises an architecture built from a few large modules — the same code spread over 300 files would pass without a single line changing. That is a fact about the measurement, not an excuse: the four dense modules are genuinely dense, and they are the ones the refactor targets.
