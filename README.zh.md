...
- **Codacy 评级为 B，而且是有意保持 B。**四项质量目标中有三项为绿：0 个未处理问题、4 % 重复率，以及 62,72 % 的覆盖率（阈值为 60 %）。第四项没有达标——被分析的 30 个源文件中有 13 个超过了复杂度阈值。该阈值是可调的，调高它就能让数字变绿，而无需改动一行代码；但它没有被调高，因为其中四个文件确实承担了过多职责。最重的两个是 `badges.js` 和 `cloud.js`。拆分它们是下一项工作——它正是横在本项目与交换清单导出、以及与多赛季支持之间的东西。关于这项指标本身有一点需要说明：它统计的是*文件数*，因此会惩罚由少数大模块构成的架构——同样的代码若分散到 300 个文件中，无需改动一行即可达标。这是关于度量方式的事实，而不是借口：那四个高密度模块确实密度过高，重构针对的正是它们。

---

## 🔩 工程说明

运行时零依赖(仅构建时使用 esbuild);版本间零布局偏移,精确到像素测量;快速添加经性能剖析优化(中端手机约 300 毫秒 → 约 45 毫秒);可选的本地加密与 PIN 绑定(PBKDF2 + AES-GCM);旁观者模式在逻辑层锁定而非 CSS;截图由仓库内确定性脚本重新生成;519 个测试基于原生 JS 与 Node 内置测试运行器。完整细节见[英文 README](README.md)。
