...
- **Codacy 评级为 A——其四项质量目标中有一项是红的。** 基于提交 `e19d1fa` 测量，121 个文件、9 741 行。绿：重复率 6 %（目标 10 %），覆盖率 69 %——这一次 Codacy 的读数与本地 `npm run test:cov`（69.10 %）一致，目标为 60 %，即九个百分点的余量，而非此前的 2.7。红：复杂度，**被分析文件中有 44 % 超过阈值**，目标为 10 %——在 `cloud.js` 被拆成四个模块之后这个比例反而*上升*了，这恰恰说明了该指标的性质：它统计的是*文件*，因此由少数大模块构成的代码库会被惩罚。这是关于度量方式的事实，不是借口；`badges.js` 确实承担过多。还有一个徽章掩盖的数字：**11 个未处理问题**，其中 **8 个出自为本 README 新增的截图脚本**（`capture_demos.py` 中的 `subprocess` 与 `exec`——开发工具，并非发布代码），2 个是 `account.js` 的未使用变量，1 个是 `cloud-auth.js` 的误报。
