...
- 🎓 **23 步交互式教程** — 一次引导之旅，你将*亲自执行*真实操作，全程运行在结束后会撤销所有改动的沙盒中。
- 🏅 **一个讲述你收藏历程的徽章页** — 50 枚徽章分为 6 个家族：收集之路、完整套组、闪卡、颜色、热爱，以及由你亲自确认的真实经历。带头衔的进度圆环、始终突出最接近解锁（或你钉选为目标）的「下一枚徽章」卡片、从 1 到 101 张的里程碑阶梯、解锁日期，以及徽章到手时真正的庆祝：合并提示、短促震动、磁贴绽放。你的收藏家卡片可导出为可分享的图片。
