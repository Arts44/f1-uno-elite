...
## ▶️ **[在线试用 → arts44.dev](https://arts44.dev/)**
...
每段演示都由 `capture_demos.py` 生成，与静态截图使用同一个确定性种子；静态截图在两次运行之间**没有任何可见差异**——每个动画都会在拍摄前被冻结到选定的相位。阈值有明确数值，一条命令即可验证（`compare_captures.py`）；逐字节一致做不到——仍会残留几十个像素、幅度 10/255 的抗锯齿噪声。GIF 为 33.3 fps；60 fps 版本在旁边：[快速添加](screenshots/demo-quick-add.mp4) · [导航](screenshots/demo-nav.mp4) · [徽章](screenshots/demo-badges.mp4) · [赛季](screenshots/demo-seasons.mp4)。
...
| 测试 | **Node 内置测试运行器**（`node --test`）— 955 个测试，未使用测试框架 |
...
**解法：** 业务逻辑经过重构，可脱离浏览器独立运行，并由 **Node 内置运行器上的 955 个测试**覆盖——没有测试依赖，也不产生真实网络请求。CI 还会重新构建产物包，一旦已提交的构建产物过期即告失败。

---

### 测试覆盖了什么 —— 以及没有覆盖什么

955 项测试,跑在 Node 内置的测试运行器上,不用任何框架。说清楚边界,比数字本身更重要:
...
npm test        # 955 个测试，node --test，无需框架
```
...
- **主页带有一个无 Cookie 的访问计数器；应用本身不加载任何东西。** 域名根目录的门户页包含 Cloudflare Web Analytics——无 Cookie、无跨站跟踪，只统计访问量。应用（`/app/`）仍然加载**零外部资源**，只要出现一个，测试套件就会让构建失败。唯一被允许的例外在同一测试中逐文件点名。
- **首页有经过实测的体积预算——120 KB，由测试守住。** 正是 Cloudflare 的访问统计揭示了首页很慢：它的截图是 LCP 元素，真实流量 P90 达 5 091 毫秒，占页面体积的 89%。修复方式是按实际显示尺寸生成 WebP 变体、为 LCP 加上 `fetchpriority`、并对 720 px 以下隐藏的那张截图启用延迟加载——都是实测而非推断：移动端从 277 KB 降到 **73 KB**，桌面端降到 **99 KB**。这些变体由与 README 截图相同的确定性脚本产出，每次运行逐字节一致。这里的讽刺值得记下：页面上唯一的外部脚本，恰恰是让我们发现它很慢的那个。
...
- **登录码与反馈通知现已从一个已验证的域名发出**（`noreply@arts44.dev`——SPF、DKIM、MX 与 DMARC 自 2026-08-13 起就位）。全新域名没有任何发信信誉，首批实测也直言不讳：上线一小时内，发往 Proximus 的邮件被**直接拒收**（`554 … poor reputation of a domain used in message transfer`，标记为暂时性），发往 Gmail 的邮件**进了垃圾箱**。这正是无历史记录域名的预期表现——随着发信量增加会逐步改善——但在信誉建立起来之前，验证码可能需要到垃圾邮件里去找，个别运营商甚至会直接拒收。发送失败依然不会阻断任何事：反馈无论如何都会写入数据库。
...
运行时零依赖(仅构建时使用 esbuild);11 px 字号下限，已在 5 种字体主题 × 2 种配色主题 × 320/375/桌面下逐一核验（仅一处例外：标志中 8 px 的「ÉLITE」，属于品牌标识）；布局偏移经过测量，现在确实为零：卡片高度已固定（此前有 13 种不同高度，从 246,69 到 292,69 px），骨架屏读取同一个 CSS 变量，因此完全吻合——代价是滚动增加 19,4 %，即处处预留最坏情况的成本;快速添加经性能剖析优化(中端手机约 300 毫秒 → 约 45 毫秒，单次测量，未在 CI 中复现);可选的本地加密与 PIN 绑定(PBKDF2 + AES-GCM);旁观者模式在逻辑层锁定而非 CSS;截图由仓库内确定性脚本重新生成，三段动画演示由脚本生成、可复现;955 个测试基于原生 JS 与 Node 内置测试运行器。完整细节见[英文 README](README.md)。
