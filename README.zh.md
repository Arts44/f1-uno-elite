...
| 测试 | **Node 内置测试运行器**（`node --test`）— 651 个测试，未使用测试框架 |
...
**解法：** 业务逻辑经过重构，可脱离浏览器独立运行，并由 **Node 内置运行器上的 651 个测试**覆盖——没有测试依赖，也不产生真实网络请求。CI 还会重新构建产物包，一旦已提交的构建产物过期即告失败。

---

### 测试覆盖了什么 —— 以及没有覆盖什么

651 项测试,跑在 Node 内置的测试运行器上,不用任何框架。说清楚边界,比数字本身更重要:
...
npm test        # 651 个测试，node --test，无需框架
```
...
- **Codacy 评级为 A——而它四项质量目标中有两项是红的。**绿：0 个未处理问题、4 % 重复率。红：复杂度，被分析的 30 个源文件中有 13 个超过阈值（43 %，目标为 10 %）；以及覆盖率，由测试套件测得 62,72 %（Codacy 自己的读数为 62 %，统计的文件范围略宽），目标为 60 %——已达标，但只有 2,7 个百分点的余量。上方的徽章是真实的，但它并非全貌——这正是全貌写在这里的原因。复杂度阈值是可调的，调高它就能让那一项变绿，而无需改动一行代码；但它没有被调高，因为其中四个文件确实承担了过多职责——最重的两个是 `badges.js` 和 `cloud.js`，拆分它们是下一项工作，因为它正是横在本项目与交换清单导出、以及与多赛季支持之间的东西。关于这项指标本身有一点需要说明：它统计的是*文件数*，因此会惩罚由少数大模块构成的架构——同样的代码若分散到 300 个文件中，无需改动一行即可达标。这是关于度量方式的事实，而不是借口。

---

## 🔩 工程说明

运行时零依赖(仅构建时使用 esbuild);11 px 字号下限，已在 5 种字体主题 × 2 种配色主题 × 320/375/桌面下逐一核验（仅一处例外：标志中 8 px 的「ÉLITE」，属于品牌标识）；布局偏移经过测量，现在确实为零：卡片高度已固定（此前有 13 种不同高度，从 246,69 到 292,69 px），骨架屏读取同一个 CSS 变量，因此完全吻合——代价是滚动增加 19,4 %，即处处预留最坏情况的成本;快速添加经性能剖析优化(中端手机约 300 毫秒 → 约 45 毫秒，单次测量，未在 CI 中复现);可选的本地加密与 PIN 绑定(PBKDF2 + AES-GCM);旁观者模式在逻辑层锁定而非 CSS;截图由仓库内确定性脚本重新生成，三段动画演示为手工录制;651 个测试基于原生 JS 与 Node 内置测试运行器。完整细节见[英文 README](README.md)。
