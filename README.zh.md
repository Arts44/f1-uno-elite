...
| 测试 | **Node 内置测试运行器**（`node --test`）— 921 个测试，未使用测试框架 |
...
**解法：** 业务逻辑经过重构，可脱离浏览器独立运行，并由 **Node 内置运行器上的 921 个测试**覆盖——没有测试依赖，也不产生真实网络请求。CI 还会重新构建产物包，一旦已提交的构建产物过期即告失败。

---

### 测试覆盖了什么 —— 以及没有覆盖什么

921 项测试,跑在 Node 内置的测试运行器上,不用任何框架。说清楚边界,比数字本身更重要:
...
npm test        # 921 个测试，node --test，无需框架
```
...
运行时零依赖(仅构建时使用 esbuild);11 px 字号下限，已在 5 种字体主题 × 2 种配色主题 × 320/375/桌面下逐一核验（仅一处例外：标志中 8 px 的「ÉLITE」，属于品牌标识）；布局偏移经过测量，现在确实为零：卡片高度已固定（此前有 13 种不同高度，从 246,69 到 292,69 px），骨架屏读取同一个 CSS 变量，因此完全吻合——代价是滚动增加 19,4 %，即处处预留最坏情况的成本;快速添加经性能剖析优化(中端手机约 300 毫秒 → 约 45 毫秒，单次测量，未在 CI 中复现);可选的本地加密与 PIN 绑定(PBKDF2 + AES-GCM);旁观者模式在逻辑层锁定而非 CSS;截图由仓库内确定性脚本重新生成，三段动画演示由脚本生成、可复现;921 个测试基于原生 JS 与 Node 内置测试运行器。完整细节见[英文 README](README.md)。
