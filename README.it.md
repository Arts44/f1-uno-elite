...
Ogni demo è generata da `capture_demos.py` con lo stesso seed deterministico delle catture fisse, **senza alcuna differenza visibile tra due esecuzioni**: ogni animazione è congelata a una fase scelta prima di ogni scatto. La soglia è numerica e verificabile con un comando (`compare_captures.py`); il byte esatto non è raggiungibile — resta un rumore di antialiasing di poche decine di pixel a 10 livelli su 255. I GIF girano a 33,3 fps; la versione a 60 fps è accanto: [aggiunta rapida](screenshots/demo-quick-add.mp4) · [navigazione](screenshots/demo-nav.mp4) · [badge](screenshots/demo-badges.mp4) · [stagioni](screenshots/demo-seasons.mp4).
...
| Test | **Test runner integrato di Node** (`node --test`) — 955 test, nessun framework di test |
...
| Interfaccia | `render.js` (griglia, scheda carta), `stats.js`, `badges.js`, `pin.js` (impostazioni) |
...
**Soluzione:** la logica è stata fattorizzata per essere indipendente dal browser ed è coperta da **955 test sul runner integrato di Node** — nessuna dipendenza di test, nessuna rete reale. La CI ricostruisce anche il bundle e fallisce se l'artefatto committato è obsoleto.

---

### Cosa coprono i test — e cosa no

955 test sull'esecutore integrato di Node, senza framework. Essere precisi sul confine conta più del numero:
...
npm test        # 955 test, node --test, senza framework
```
...
- **La home page porta un contatore di pubblico senza cookie; l'applicazione non carica nulla.** La pagina vetrina alla radice del dominio include Cloudflare Web Analytics — niente cookie, niente tracciamento cross-site, solo conteggi di visite. L'applicazione (`/app/`) continua a caricare **zero risorse esterne**, e una suite di test rompe la build se ne compare una sola. L'unica eccezione ammessa è nominata, file per file, nello stesso test.
- **La pagina vetrina ha un budget di peso misurato — 120 KB, tenuto da un test.** È stato il contatore di Cloudflare a rivelarne la lentezza: le sue schermate erano l'elemento LCP a 5 091 ms sul P90 di traffico reale, l'89 % del peso della pagina. Corretto con varianti WebP dimensionate per la loro visualizzazione reale, `fetchpriority` sull'LCP e caricamento differito sulla schermata nascosta sotto 720 px — misurato, non dedotto: mobile da 277 KB a **73 KB**, desktop a **99 KB**. Le varianti escono dallo stesso script deterministico delle immagini del README. L'ironia merita di essere annotata: l'unico script esterno della pagina è proprio ciò che ha permesso di scoprire che era lenta.
...
- **I codici di accesso e le notifiche delle opinioni partono ora da un dominio verificato** (`noreply@arts44.dev` — SPF, DKIM, MX e DMARC attivi dal 13/08/2026). Un dominio nuovo non ha reputazione di invio, e le prime misure lo dicono senza giri di parole: entro un'ora, un messaggio verso Proximus è stato **rifiutato** (`554 … poor reputation of a domain used in message transfer`, segnalato transitorio) e un messaggio verso Gmail è **finito nello spam**. È il comportamento atteso di un dominio senza storia — migliora con il volume — ma finché la reputazione si costruisce, un codice può dover essere ripescato dalla posta indesiderata, o essere rifiutato da qualche operatore. Un invio fallito continua a non bloccare nulla: l'opinione viene comunque salvata nel database.
