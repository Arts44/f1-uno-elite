...
- 🎓 **Tutorial interattivo, 28 passi in 5 capitoli** — un capitolo per pagina, nell’ordine delle schede. Una visita guidata in cui *esegui* le azioni reali, in un ambiente isolato che annulla ogni modifica alla fine.
...
| Test | **Test runner integrato di Node** (`node --test`) — 519 test, nessun framework di test |
...
npm test        # 519 test, node --test, senza framework
```
...
- **Il voto Codacy è B, e resta B di proposito.** Dei quattro obiettivi di qualità che Codacy segue, due sono verdi — 0 issue aperte e 4 % di duplicazione — e la copertura di righe si attesta al 62,72 % contro una soglia del 60 %, misurata dalla suite di test stessa. Il quarto no — 13 dei 30 file sorgente analizzati superano la soglia di complessità. Quella soglia è regolabile, e alzarla farebbe diventare verde il numero senza cambiare una riga di codice; non è stata alzata, perché quattro di quei file fanno davvero troppo. I due più pesanti sono `badges.js` e `cloud.js`. Scinderli è il prossimo lavoro — è ciò che separa il progetto dall'esportazione della lista di scambio, e dal supporto multi-stagione. Una riserva sulla metrica stessa: conta *file*, quindi penalizza un'architettura fatta di pochi grandi moduli — lo stesso codice distribuito su 300 file passerebbe senza cambiare una riga. È un fatto sulla misura, non una scusa: i quattro moduli densi lo sono davvero, ed è a loro che il refactor mira.

---

## 🔩 Note di ingegneria

Zero dipendenze a runtime (solo esbuild, in build); zero spostamenti di layout, misurati al pixel tra le versioni; l'aggiunta rapida profilata e ottimizzata (~300 ms → ~45 ms su un telefono medio); cifratura locale opzionale legata al PIN (PBKDF2 + AES-GCM); modalità spettatore bloccata nella logica, non nel CSS; screenshot rigenerati da uno script deterministico nel repo, le tre demo animate registrate a mano; 519 test in JS vanilla con il runner integrato di Node. Dettagli completi nel [README inglese](README.md).
