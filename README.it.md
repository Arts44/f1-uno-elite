...
| Test | **Test runner integrato di Node** (`node --test`) — 866 test, nessun framework di test |
...
**Soluzione:** la logica è stata fattorizzata per essere indipendente dal browser ed è coperta da **866 test sul runner integrato di Node** — nessuna dipendenza di test, nessuna rete reale. La CI ricostruisce anche il bundle e fallisce se l'artefatto committato è obsoleto.

---

### Cosa coprono i test — e cosa no

866 test sull'esecutore integrato di Node, senza framework. Essere precisi sul confine conta più del numero:
...
npm test        # 866 test, node --test, senza framework
```
