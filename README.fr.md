...
## ▶️ **[Essayer en ligne → arts44.dev](https://arts44.dev/)**
...
- 🧭 **Une barre de navigation à pastille** — la pilule, son encoche et la pastille forment un seul chemin SVG recalculé image par image depuis une horloge unique, si bien qu'aucune ne dérive par rapport aux autres. Glissable, accessible au clavier, et respectant `prefers-reduced-motion`.
...
| Couche | Modules |
|---|---|
| État & données | `storage.js` (localStorage, scopé par saison, migration v1→v2), `data.js`, `history.js` |
| Interface | `render.js` (grille, fiche carte), `stats.js`, `badges.js`, `pin.js` (réglages) |
...
**Solution :** la logique a été factorisée pour être indépendante du navigateur et couverte par **955 tests sur le runner intégré de Node** — aucune dépendance de test, aucun réseau réel. La CI reconstruit aussi le bundle et échoue si l'artefact commité est périmé.

---

### Ce que les tests couvrent — et ce qu'ils ne couvrent pas

955 tests sur le lanceur intégré de Node, sans framework. Être précis sur la frontière compte plus que le nombre :
...
npm test        # 955 tests, node --test, sans framework
```
...
- **La page d'accueil porte un compteur d'audience sans cookie ; l'application, elle, ne charge rien.** La page vitrine à la racine du domaine inclut Cloudflare Web Analytics — pas de cookie, pas de pistage inter-sites, juste des comptes de visites. L'application (`/app/`) charge toujours **zéro ressource externe**, et une suite de tests casse le build si une seule apparaît. L'unique exception autorisée est nommée, fichier par fichier, dans le même test.
- **La page d'accueil a un budget de poids mesuré — 120 Ko, tenu par un test.** C'est le compteur d'audience de Cloudflare qui a révélé sa lenteur : ses captures étaient l'élément LCP à 5 091 ms au P90 sur du trafic réel, soit 89 % du poids de la page. Corrigé par des variantes WebP dimensionnées pour leur affichage réel, `fetchpriority` sur le LCP et un chargement différé sur la capture masquée sous 720 px — mesuré et non déduit : le mobile est passé de 277 Ko à **73 Ko**, le desktop à **99 Ko**. Les variantes sortent du même script de capture déterministe que celles du README, identiques à l'octet d'une exécution à l'autre. Le seul script externe de la page est précisément ce qui a permis de découvrir qu'elle était lente.
...
- **Les codes de connexion et les notifications d'avis partent désormais d'un domaine vérifié** (`noreply@arts44.dev` — SPF, DKIM, MX et DMARC en place depuis le 13/08/2026). Un domaine neuf n'a aucune réputation d'envoi, et les premières mesures le disent sans détour : dans l'heure, un message vers Proximus a été **rejeté** (`554 … poor reputation of a domain used in message transfer`, marqué transitoire) et un message vers Gmail est **arrivé en indésirables**. C'est le comportement attendu d'un domaine sans historique — ça s'améliore avec le volume — mais tant que la réputation se construit, un code peut devoir être repêché dans les indésirables, voire être refusé par certains FAI. Un envoi raté ne bloque toujours rien : l'avis est enregistré en base dans tous les cas.
